...
def scenario_trace_bundle(
    ...
    *,
    ...
    ctx = make_run_context(seed=seed, scenario_id=scenario_id, system_id=system_id)
    trace = run_agent(system_id, scenario_id, ctx)
...
def unsupported_claim_spike_sequence(system_id: str | None = None) -> list[dict[str, Any]]:
    ...
    return [scenario_trace_bundle(sid, "unsupported_claim_spike", seed=42)]


def tool_scope_violation_sequence(system_id: str = "sys-agt-phish-008") -> list[dict[str, Any]]:
    return [scenario_trace_bundle(system_id, "tool_scope_violation", seed=43)]


def provider_latency_regression_sequence(system_id: str = "sys-agt-cs-002") -> list[dict[str, Any]]:
    return [scenario_trace_bundle(system_id, "provider_latency_regression", seed=44)]
